...
export default function Error({
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center">
      ...
      <h2 className={`text-3xl font-bold mb-3 ${theme === "light" ? "text-gray-800" : "text-gray-100"}`}>
        Something went wrong
      </h2>

      <p className={`text-lg mb-8 max-w-md mx-auto ${theme === "light" ? "text-gray-600" : "text-gray-300"}`}>
        We've encountered an unexpected error. Our team has been notified and is working to fix the issue.
      </p>

      <div className="flex flex-wrap gap-4 justify-center">
        <button onClick={() => reset()} className="cta-button cta-secondary flex items-center">
      ...
      >
        <h3 className={`text-lg font-medium mb-2 ${theme === "light" ? "text-gray-800" : "text-gray-200"}`}>
          Need help?
        </h3>
        <p className={`mb-4 ${theme === "light" ? "text-gray-600" : "text-gray-400"}`}>
          If this issue persists, please contact our support team or try one of these options:
        </p>
        <ul className="space-y-2 text-left">
          ...
          <li>
            ...
            >
              <i className="fab fa-discord mr-2"></i> Join our Discord for support
            </a>
          </li>
        </ul>
      </div>
